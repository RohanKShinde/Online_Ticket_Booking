...
<body>
    ...
    <form action="" method="post" class="firstform">
        ...
        Stadium: <select name="src_name" id="src_id">
            <option value="Default">Choose Stadium</option>
            <option value="Mumbai">Mumbai-Wankhede</option>
            <option value="Mohali">Mohali-PCA Stadium</option>
            <option value="Banglore">Bangalore-M. Chinnaswamy Stadium</option>
            <option value="Pune">Pune-MCA Stadium</option>
            <option value="Kolkata">Kolkata-Eden Gardens</option>

        </select><br><br>
        Date of Match: <input type="date" name="date_name" id="date_id">
        <br><br>
        <?php
        session_start();
    ...
    <br><br>
    <form action="USER info.php" method="post" class="secondform">
        <?php
        if (isset($_POST['submit'])) {
            ...
            $db = mysqli_connect('localhost', 'root', '', 'ticket') or die("Could not connect to Database");

            $querry = "SELECT * FROM ticket_details WHERE stadium='$frm' ";
